...
$$P(k) = \left(1 + \left(\frac{k}{k_*}\right)^2\right)^{-p/2}$$
   ...
   A 2D slope $p = 3$ gives a 1D isotropic slope of $k^{-2}$ and $k_*$ sets the transition wavenumber. 

4. **Spectral tapering** — apply a smooth exponential taper to the initial condition spectrum amplitude: $\exp(-2(k/k_c)^4)$ to supress energy near the grid scale.
